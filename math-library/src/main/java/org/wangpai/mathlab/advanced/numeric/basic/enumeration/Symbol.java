package org.wangpai.mathlab.advanced.numeric.basic.enumeration;
...
public enum Symbol {
    ...
    DOT(".", 210),
    /**
     * 注意：此符号的底层字符串表示与 Symbol.SUBTRACT 相同，无法通过 Symbol.getEnum 作区分，使用时应作额外判断
     */
    NEGATIVE("-", 211),
    /******************** 元运算符 ********************/
    /******************* 运算符 *******************/
    ...
    WRAP(System.lineSeparator(), 1001), // 换行
    ...
    /**
     * 因为枚举具有唯一性，而 String 类型与 Symbol 类型具有多对多的关系，
     * 因此，此方法进行从 String 到 Symbol 之间的转化时，会丢失原 String 的信息
     *
     * 如果遇到一个 String 类型对应多个 Symbol 类型的情况，只会返回第一个。
     * 比如，减号与负号的 String 符号是同一个，但使用本方法时只会当成是减号
     *
     * @since 2021-7-30
     */
    ...
    /**
     * Meta Operator：元运算符（不具有操作数的运算符，可视为一元运算符）
     *
     * 注意：小数点属于元运算符，但它不属于 N 元运算符
     *
     * @since 2021-8-2
     */
