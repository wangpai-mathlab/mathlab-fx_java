...
import org.wangpai.exception.unchecked.failed.OverflowUncheckedException;

/**
 * 表示一个按位存储的任意进制的整数，但是位数是固定的
 *
 * @since 2022-11-17
 */
@ToString
@Accessors(chain = true)
public class FixedRadix implements Cloneable {
    /**
     ...
     * @since 2022-11-17
     */
    public FixedRadix(int[] digits, int radix, boolean isNegative) {
        if (radix <= 1) {
     ...
     * @since 2022-11-17
     */
    public static FixedRadix generateRadixWnc(int[] digits, int radix, boolean isNegative) {
        var result = new FixedRadix();
     ...
     * @since 2022-12-25
     */
    public static FixedRadix generateZero(int radix, int length) {
        var result = new FixedRadix();
     ...
     */
    @Override
    public FixedRadix clone() {
        try {
            FixedRadix cloned = (FixedRadix) super.clone();
     ...
     * @since 2022-12-24
     */
    public boolean equalsByMathValue(FixedRadix other) {
        if (this == other) {
            return true;
        }
        if (other == null) {
...
//        return false;
     ...
     * @since 2022-12-24
     */
    public boolean equalsByData(FixedRadix other) {
        if (this == other) {
            return true;
        }
        if (other == null) {
    ...
    public boolean equals(Object other) {
        ...
        if (other instanceof FixedRadix otherRadix) {
     ...
     * @since 2022-12-24
     */
    public FixedRadix increaseOne() throws LogicalException {
        if (this.isFinal) {
     ...
     * @since 2022-12-24
     */
    public FixedRadix decreaseOne() throws LogicalException {
        if (this.isFinal) {
            throw new LogicalException("错误：不能对常量【" + this.name + "】进行自增");
        }
    ...
    public void increase1Abs() {
        ...
        if (index < 0) { // 说明递增之前的数刚好是同一位数下的最大数，此时如果继续操作就会发生溢出
            throw new OverflowUncheckedException("错误：自增失败，FixedRadix 已达到最大");
        }
    }
