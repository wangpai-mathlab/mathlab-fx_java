...
class BinaryEncodingTest {
     ...
     */
    @Test
    void encodeWithNoCheckForCom() throws OverflowException {
        /**
         * 实现“排列组合”中，组合选择操作
         *
         * 这个类必须放在方法的前部
        ...
        class Combination {
            ...
            public static void combination0(int[] combinationSource, int[] singleCombination, LinkedList<int[]> combinations,
                                            int nextOrder, int sourceStart) throws OverflowException {
                if (nextOrder == singleCombination.length) {
//            combinations.add(Arrays.copyOf(singleCombination, singleCombination.length));
                    long encoded = BinaryEncoding.encodeWithNoCheckForCom(
                            combinationSource.length, singleCombination, 0).toLong();
             ...
             * @since 2022-10-22
             */
            public static List<int[]> combination(int[] combinationSource, int choiceNum) throws OverflowException {
