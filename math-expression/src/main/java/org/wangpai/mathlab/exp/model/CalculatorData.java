...
public final class CalculatorData implements Cloneable {
    ...
    public boolean loadOpnd() {
        ...
        var rational = new Decimal(this.opndBuff.toArray(Symbol[]::new)).toRational();
        this.opnds.push(rational);
        this.calculatedExp.push(rational);
        this.opndBuff.clear();
        return true;
    }
     ...
     */
    public Operand oneTimeCalculation() throws MathlabCheckedException {
    ...
    public static Operand oneTimeCalculation(Operand opndLeft, Operator optr, Operand opndRight)
        ...
        if (opndLeft instanceof Rational
                && opndRight instanceof Rational) {
            /**
             * 介于可读性及因反射的危险性带来的复杂性，此处不要使用反射
             */
            switch (optr.getSymbol()) {
                case ADD:
                    return RationalOperation.add((Rational) opndLeft, (Rational) opndRight);
                case SUBTRACT:
                    return RationalOperation.subtract((Rational) opndLeft, (Rational) opndRight);
                case MULTIPLY:
